...
# -*- coding: utf-8 -*-
"""
IBM i I/O Stress Test - Version Python
Outil de stress test I/O disque pour démonstrations commerciales IBM Power
Génère une charge intensive sur les disques
"""
...
class IOStressTest:
    """Classe pour effectuer des tests de stress I/O sur IBM i"""
    
    def __init__(self, duration: int, directory: str, file_size_mb: int = 100, 
                 operation: str = 'mixed'):
        """
        Initialise le test de stress I/O
    ...
    def run_stress_test(self) -> dict:
        ...
        print(f"🚀 Démarrage du test de stress I/O")
...
def worker_io_process(duration: int, directory: str, file_size_mb: int, 
    ...
    stress_test = IOStressTest(duration, str(worker_dir), file_size_mb, operation)
    stats = stress_test.run_stress_test()
...
def run_multi_process_io_stress(duration: int, num_processes: int, directory: str,
                                 file_size_mb: int, operation: str):
    """
    Lance un test de stress I/O sur plusieurs processus
    ...
    """
    print("\n" + "=" * 60)
    print("🎯 TEST DE STRESS I/O MULTI-PROCESSUS")
    print("=" * 60)
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        ...
        for i in range(num_processes):
            result = pool.apply_async(
                worker_io_process,
                (duration, directory, file_size_mb, operation, i + 1)
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Outil de stress test I/O pour IBM i - Démonstrations commerciales',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemples d'utilisation:
  # Test d'écriture simple pendant 60 secondes
  python ibmi_stress_io.py --duration 60 --operation write
  
  # Test de lecture avec fichiers de 200 MB
  python ibmi_stress_io.py --duration 120 --operation read --file-size 200
  
  # Test mixte sur 4 processus
  python ibmi_stress_io.py --duration 300 --operation mixed --processes 4
  
  # Test intensif avec nettoyage automatique
